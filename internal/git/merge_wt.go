...
type MergeRequest struct {
	...
	// StrategyOption, if non-empty, is passed as "git merge -X <option>",
	// e.g. "ours" or "theirs".
...
func (w *Worktree) Merge(ctx context.Context, req MergeRequest) (retErr error) {
	var args []string
	if req.NoEdit {
	...
	if req.Autostash {
		...
		defer func() {
			...
			w.checkAutostashPop(ctx, branch, &retErr)
		}()
	}
...
func (w *Worktree) handleMergeError(ctx context.Context, err error) error {
	return handleInterruptedOp(ctx, w, "merge", err, w.MergeState,
		// No merge state: the merge failed for another reason.
		func(stateErr error) bool { return errors.Is(stateErr, ErrNoMerge) },
		func(state *MergeState, err error) error {
...
func (w *Worktree) MergeContinue(ctx context.Context, opts *MergeContinueOptions) error {
	...
	// Use "git commit" instead of "git merge --continue"
	// to reuse Commit's index-lock retry and editor handling.
	if err := w.Commit(ctx, CommitRequest{NoEdit: !opts.Edit}); err != nil {
		return w.handleMergeError(ctx, err)
	}
	return nil
}
