...
func TestMerge_autostashConflict(t *testing.T) {
	fixture, err := gittest.LoadFixtureScript([]byte(text.Dedent(`
		as 'Test <test@example.com>'
	...
	// Conflicts with main's change to base.txt when the stash pops.
	...
	assert.NotErrorAs(t, err, new(*git.MergeInterruptError),
		"merge should not return MergeInterruptError for autostash conflict")
...
func TestMerge_failureWithoutMergeState(t *testing.T) {
	wt, dir := conflictingWorktree(t)

	ctx := t.Context()

	// Dirty the file that the merge would change.
	require.NoError(t, os.WriteFile(
		filepath.Join(dir, "shared.txt"),
		[]byte("Uncommitted change to shared"), 0o644))
	...
	var mergeErr *git.MergeInterruptError
	assert.NotErrorAs(t, err, &mergeErr,
		"a merge that never started must not be an interrupt")
	_, ok := errors.AsType[git.InterruptError](err)
	assert.False(t, ok, "a merge that never started must not be an interrupt")
	...
	bs, err := os.ReadFile(filepath.Join(dir, "shared.txt"))
	require.NoError(t, err)
	assert.Equal(t, "Uncommitted change to shared", string(bs))
}
...
func TestMerge_rerereScopedPerInvocation(t *testing.T) {
	...
	// rerere must not be persisted to the repo config.
...
// conflictingWorktree creates a repository where merging main
// into the checked-out feature branch conflicts on shared.txt.
