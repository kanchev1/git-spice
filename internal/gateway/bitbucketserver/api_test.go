...
func TestPullRequestCreate_destinationBranchMissing(t *testing.T) {
...
func TestPullRequestCreate_unrelatedBadRequest(t *testing.T) {
	// A 400 on the create path that is not an argument-validation failure
	// about a missing branch must not be misclassified as
	// ErrDestinationBranchNotFound; it should surface as a generic APIError.
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
		writeJSON(t, w, http.StatusBadRequest, map[string]any{
			"errors": []map[string]any{
				{
	...
	require.Error(t, err)
	assert.NotErrorIs(t, err, ErrDestinationBranchNotFound)
}
