...
// caused by a missing destination (target) branch.
...
func checkResponse(resp *http.Response, body []byte) error {
	...
	if isDestinationBranchNotFound(resp, details) {
...
// isDestinationBranchNotFound reports whether the response describes a pull
// request creation failure caused by a missing target branch.
//
// It is deliberately conservative: it fires only for the create-PR POST
// (a 400 on a ".../pull-requests" path) and keys on branch-specific "does
// not exist" phrasing, since validation 400s often carry exceptionName: null.
func isDestinationBranchNotFound(resp *http.Response, details []APIErrorDetail) bool {
	...
	if !strings.HasSuffix(resp.Request.URL.EscapedPath(), "/pull-requests") {
		return false
	}
	...
	for _, d := range details {
		...
		if strings.Contains(text, "branch") &&
			(strings.Contains(text, "not exist") || strings.Contains(text, "does not")) {
			return true
		}
	}
	return false
