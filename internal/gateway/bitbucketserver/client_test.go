...
func TestClient_errors(t *testing.T) {
	...
	t.Run("DestinationBranchMissing", func(t *testing.T) {
		srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, _ *http.Request) {
			// Realistic validation 400: exceptionName is null, so the
			// message substring must carry the signal on its own.
			writeJSON(t, w, http.StatusBadRequest, map[string]any{
				"errors": []map[string]any{
					{
		...
		client := newTestClient(t, srv)
		// Hit a path ending in /pull-requests so the heuristic applies.
		_, err := client.post(
			t.Context(),
