...
// PR uniquely identifies a Pull Request in Bitbucket Data Center.
...
func mustPR(id forge.ChangeID) *PR {
	pr, ok := id.(*PR)
	if !ok {
		panic(fmt.Sprintf("bitbucket-server: expected *PR, got %T", id))
	}
	return pr
}
...
type PRComment struct {
	...
	// PRID is the owning pull request ID, required to update or delete the comment.
	PRID int64 `json:"pr_id,omitempty"`

	// Version is the comment's optimistic-locking version, required on update
	// and delete. A persisted version may be stale and need refetching.
...
func mustPRComment(id forge.ChangeCommentID) *PRComment {
	...
	c, ok := id.(*PRComment)
	if !ok {
		panic(fmt.Sprintf("bitbucket-server: expected *PRComment, got %T", id))
	}
	return c
}
...
// SetNavigationCommentID sets the navigation comment ID; id may be nil.
