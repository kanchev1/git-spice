...
func TestForge_ID(t *testing.T) {
	f := &Forge{}
	assert.Equal(t, "bitbucket-server", f.ID())
}

func TestForge_DisplayName(t *testing.T) {
	f := &Forge{}
	assert.Equal(t, "Bitbucket Data Center", f.DisplayName())
}
...
func TestForge_ChangeTemplatePaths(t *testing.T) {
	f := &Forge{}
	assert.Contains(t, f.ChangeTemplatePaths(), "PULL_REQUEST_TEMPLATE.md")
}
...
func TestForge_OpenRepository_requiresURL(t *testing.T) {
	...
	_, err := f.OpenRepository(
		...
		&RepositoryID{url: "", projectKey: "KEY", slug: "repo"},
	)
	require.Error(t, err)
