...
func (cmd *mainCmd) AfterApply(
	...
	return errors.Join(
		...
		kctx.BindSingletonProvider(func(
			...
			wt *git.Worktree,
			store *state.Store,
		) (ContinuationHandler, error) {
			return &continuation.Handler{
				Log:      log,
				Worktree: wt,
				Store:    store,
				Parser:   kongCommandParser{kong: kctx.Kong},
			}, nil
		}),
		kctx.BindSingletonProvider(func(
			log *silog.Logger,
...
type AutostashHandler interface {
	...
	BeginMergeAutostash(ctx context.Context, method spice.RestackMethod, opts *autostash.Options) (func(*error), error)
	RestoreAutostash(ctx context.Context, stashHash string) error
}

var _ AutostashHandler = (*autostash.Handler)(nil)

// kongCommandParser adapts [kong.Kong] to [continuation.CommandParser].
type kongCommandParser struct {
	kong *kong.Kong
}

func (p kongCommandParser) Parse(args []string) (continuation.Command, error) {
	kctx, err := p.kong.Parse(args)
	if err != nil {
		return nil, err
	}
	return kongCommand{kctx}, nil
}

// kongCommand adapts a parsed [kong.Context] to [continuation.Command].
type kongCommand struct {
	kctx *kong.Context
}

func (c kongCommand) Run(ctx context.Context) error {
	return c.kctx.Run(ctx)
}
