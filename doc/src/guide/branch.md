...
This setting only controls how a restack replays a branch onto its base;
it has no effect on $$gs branch merge$$ or $$gs stack merge$$,
which merge a branch's change request on the forge.
